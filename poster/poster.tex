...
\begin{block}{Successful Applications}
	...
	The concept of dynamic demand-driven deployment has been used 
	for lumber mills \cite{yanex_agent-based_2009}, coupling building efficiency with weather \cite{gonzalez_detailed_2002, kusiak_data-driven_2010},
	and in the airline industry \cite{shebalov_practical_2009}.
...
\begin{itemize}
	\item Non-Optimizing (NO)
	\begin{itemize}
		\item Predict based on historical supply-demand data
		\item Does not attempt to meet demand optimally
		\item Fast execution time with limited precision
		\item ex) Autoregressive Moving Average (ARMA), Autoregressive Conditional Heteroskedastic (ARCH)
	\end{itemize}
	\item Deterministic Optimizing (DO)
	\begin{itemize}
		\item Minimize or Maximize an objective function with set of constraints
		\item Replicable
		\item ex) Global Change Assessment Model (GCAM), MARKet and ALlocation (MARKAL)
	\end{itemize}
	\item Stochastic Optimizing (SO)
	\begin{itemize}
		\item Probabilistic search into the objective function or constraint models
		\item Uncertainty in addition to mean
		\item Uses random samples from probability distributions
		\item ex) Markov Switching-Model, Gaussian Process Regression
	\end{itemize}
\end{itemize}
